...
export function ScoreLineChart({ data, height = 160 }: { data: LinePoint[]; height?: number }) {
  ...
  const w = 100; // percentage units for responsive
  const h = height;
  ...
  return (
    <svg width="100%" height={h} style={{ overflow: "visible" }}>
      ...
      {gridLines.map((v) => {
        ...
        return (
          <g key={v}>
            <line x1={`${padding.left}%`} y1={y} x2={`${w - padding.right}%`} y2={y} stroke="var(--surface-raised)" strokeWidth={1} strokeDasharray="2 4" />
